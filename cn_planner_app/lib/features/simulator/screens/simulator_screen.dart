  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      setState(() {
        ...
        if (savedPlan.isNotEmpty) {
          for (var ti = 0; ti < _terms.length; ti++) {
            final term = _terms[ti];
            final key = '${term.year}_${term.term}';
            final termPlan = savedPlan[key];

            // ถ้า term นี้ไม่มีข้อมูลใน plan → ใช้ roadmap เดิม ไม่ต้องแก้
            if (termPlan == null) continue;

            // 1. Drop วิชาที่ user เคย drop ออก
            //    (วิชาที่ไม่อยู่ใน plan.codes แสดงว่าถูก drop ไปแล้ว)
            term.courses.removeWhere((c) => !termPlan.codes.contains(c.code));

            // 2. Add วิชาที่ user เคย add เข้ามา
            //    (อยู่ใน plan.codes แต่ไม่มีใน roadmap term นี้)
            final existingCodes = term.courses.map((c) => c.code).toSet();
            for (final code in termPlan.codes) {
              if (!existingCodes.contains(code)) {
                final catalogCourse = _catalogByCode[code];
                if (catalogCourse != null) {
                  term.courses.add(_copyForTerm(catalogCourse, term));
                }
                // custom course ที่ไม่อยู่ใน catalog จะข้ามไป
                // (metadata หายเพราะ simulatorplan เก็บแค่ subject_code)
              }
            }

            // 3. Apply outcomes (pass / fail)
            //    ทำหลัง add/drop เสมอ เพื่อไม่ให้ _refreshTermStatuses override
            for (var i = 0; i < term.courses.length; i++) {
              final course = term.courses[i];
              final outcome = termPlan.outcomes[course.code];
              if (outcome != null) {
                term.courses[i] = course.copyWith(outcome: outcome);
              }
            }
          }
        }
